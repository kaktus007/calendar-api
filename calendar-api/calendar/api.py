...
@app.route(CALENDAR_API_ROOT + "/date/<date>/", methods=["GET"])
def read_by_date(date: str):
    try:
        event = _event_logic.get_by_date(date)
        if event:
            raw_event = _to_raw(event)
            return raw_event, 200
        else:
            return "no event for this date", 404
    except Exception as ex:
        return f"failed to READ BY DATE with: {ex}", 404